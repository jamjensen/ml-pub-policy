...
DLQ_COL = ['HBP']
KEEP_COLS = ['Sex', 'Race', 'Obesity', 'Smokes', 'Alcohol Related Disorder','Drug Substance Disorder','Criminal Justice Status','Private Insurance']
THRESHOLD = .4
...
class knn:
    ...
    '''
    def __init__(self, data, n_neighbors, metric):
        self.model = KNeighborsClassifier(n_neighbors=n_neighbors, metric=metric, weights='uniform')
...
def buildmodel(raw_path, n_neighbors, metric):
    '''
    ...
    neigh_df = format_df(df, KEEP_COLS)

    knn_model = knn(neigh_df, n_neighbors, metric)

    x_train, y_train = pipeline.get_x_y_df( \
        knn_model.data, DLQ_COL)

    knn_model.train(x_train, y_train)
    knn_model.predict(None)
